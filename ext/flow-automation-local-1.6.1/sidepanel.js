...
async function doStart() {
  try {
    ...
    if (app.isRunning) {
      await broadcastFlow('PAUSE_AUTOMATION', {});
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  if (a === 'LOG') {
    log(message.type || 'info', message.message || '');
    ...
    app.tableRows = Array.isArray(d.rows) ? d.rows : [];
    app.tableMeta = { autoDownload: !!d.autoDownload, autoRename: !!d.autoRename };
