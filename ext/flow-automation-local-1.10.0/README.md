...
## 0. Có gì mới trong 1.10.0 — HẾT CẢNH BOT ĐỨNG VÌ "MẤT MẠNG" GIẢ

### Lỗi trước đây

Đang chạy ngon, bỗng khung Log ngập kín một dòng lặp mỗi 5 giây, mỗi tab một dòng:

```
[08:25:03] [T1] ⚠️ Mất mạng! Bot đang ngủ đông chờ kết nối...
[08:25:03] [T2] ⚠️ Mất mạng! Bot đang ngủ đông chờ kết nối...
[08:25:03] [T3] ⚠️ Mất mạng! Bot đang ngủ đông chờ kết nối...
```

Trong khi **máy vẫn vào mạng bình thường** — web vẫn lướt được, Flow vẫn mở được.
Bot thì chết hẳn: không tạo, không tải, không đổi tên, và **không có cách nào bỏ qua**.

### Nguyên nhân

Bản 1.9.0 chặn vòng chạy chính bằng đúng một dòng:

```js
if (!navigator.onLine) { ...ngủ 5 giây rồi thử lại... }
```

`navigator.onLine` **không kiểm tra Internet**. Nó chỉ đọc lại trạng thái card mạng mà
Chrome nhận từ hệ điều hành (trên Windows là NLA). Nó kẹt ở `false` trong khi mọi
request thật vẫn chạy bình thường, ở rất nhiều tình huống đời thường:

- **máy ngủ / hibernate rồi thức dậy** — Chrome chốt "offline" cho tới sự kiện card
  mạng kế tiếp mới mở lại. Đây là ca hay gặp nhất với người chạy bot qua đêm;
...
- **chạy trên VPS / Remote Desktop** — loại card mạng ở đó Chrome thường không nhận
  ra, nên báo mất mạng **vĩnh viễn**;
- ai đó để DevTools → Network → throttling ở **Offline** cho tab đó.

Vì đây là trạng thái của **cả trình duyệt** nên mọi tab cùng báo một lúc — đúng như
log ở trên. Dòng "Đã kết nối Google Flow" ở đầu panel không mâu thuẫn: nó chỉ thử gửi
message tới content script, hoàn toàn không đụng tới mạng.

### Cách sửa

`navigator.onLine = false` giờ chỉ là **nghi ngờ**, không còn là kết luận. Khi nghi,
tiện ích gọi thử một request thật tới **chính Flow** (cùng origin, `no-store`, có
cache-buster, timeout 8 giây) rồi mới quyết:

| Tình huống | 1.9.0 | 1.10.0 |
|---|---|---|
| Chrome báo sai, vẫn gọi được Flow | bot đứng vĩnh viễn, log lặp mỗi 5 giây | **chạy tiếp**, ghi đúng **một** dòng nói rõ là cảnh báo sai |
| Mất mạng thật | log lặp mỗi 5 giây × mỗi tab | ngủ đông, log **gộp** — một dòng lúc mất, nhắc lại tối đa 1 phút/lần |
| Có mạng trở lại | im lặng, không báo | báo **một** dòng "🌐 Có mạng trở lại sau N giây" |
| Lý do mất mạng | không có | ghi rõ: *hết thời gian chờ (quá 8 giây)* / *không gửi được request tới Flow (mất mạng hoặc DNS lỗi)* |

Thêm hai điều nhỏ nhưng quan trọng:

- Tiện ích **nghe sự kiện `online` của trình duyệt** nên tỉnh lại ngay khi có mạng,
  không phải đợi hết nhịp dò.
- Máy chủ trả **404 vẫn tính là có mạng** — muốn nhận được mã lỗi thì request đã phải
  đi tới nơi rồi.

Phép dò có giãn nhịp nên không tốn băng thông: 15 giây/lần khi đang mất mạng thật,
60 giây/lần khi đã biết là báo động giả, và **không dò gì cả** khi `navigator.onLine`
đang bình thường.

### Cách dùng trong giao diện

**Cài đặt → Nhịp chạy & Retry → Chốt chặn mất mạng**, hai lựa chọn:

- **Tự động (mặc định, khuyến nghị)** — nghi mất mạng thì gọi thử Flow rồi mới dừng.
  Gần như ai cũng nên để nguyên mức này.
- **Bỏ qua hẳn** — không bao giờ dừng vì báo mất mạng. Dành cho máy thuộc nhóm **luôn
  báo sai** (VPS, Remote Desktop). Lưu ý: chọn mức này thì lúc mất mạng thật bot sẽ
  **văng hàng loạt lỗi tạo/tải** thay vì đứng chờ — đổi lại là nó không bao giờ đứng oan.

Báo cáo chẩn đoán (Cài đặt → Chẩn đoán giao diện → tải báo cáo) nay có thêm một dòng
ghi mức chốt chặn đang đặt và giá trị `navigator.onLine` ngay lúc xuất báo cáo — hữu
ích khi cần xác định máy có thuộc nhóm báo sai hay không.

### Giới hạn phải nói thẳng

- Tiện ích **không sửa được** trạng thái sai của Chrome, chỉ **không tin** nó nữa. Biểu
  tượng Wi-Fi của Windows và ô "không có Internet" của Chrome vẫn có thể hiển thị sai
  cho tới khi có sự kiện card mạng mới (rút/cắm Wi-Fi, hoặc khởi động lại Chrome —
  F5 tab **không** đủ vì trạng thái nằm ở tiến trình chính của trình duyệt).
- Phép dò chỉ chứng minh **tới được flow.google.com**, không chứng minh phiên đăng nhập
  còn hạn hay hạn mức tạo của tài khoản còn hay hết. Hai thứ đó vẫn báo qua lỗi riêng.
- Chốt này chỉ canh **vòng chạy chính**. Một request lẻ vẫn hỏng được vì mạng chập
  chờn; phần đó do cơ chế Retry sẵn có xử lý.

---

## 0b. Bản 1.9.0 — TỰ BÁO KHI CÓ BẢN MỚI (vẫn còn)

Trước 1.9.0, mỗi lần có bản mới thì người dùng **không có cách nào biết** ngoài việc
được nhắn riêng. Với người dùng thứ hai (bạn bè, máy khác) thì họ cứ chạy bản cũ mãi,
kể cả khi bản mới đã sửa đúng lỗi họ đang gặp.

Cần nói thẳng một giới hạn không vượt qua được: tiện ích này cài kiểu **Tải tiện ích
đã giải nén** (Load unpacked), và Chrome **không** có cơ chế tự cập nhật cho kiểu cài
đó — cơ chế auto-update chỉ chạy với tiện ích cài từ Chrome Web Store. Extension cũng
**không được phép tự ghi đè file của chính nó** trên ổ đĩa. Nên **không có "tự cập nhật
âm thầm" thật sự**; việc giải nén thay thư mục vẫn phải làm tay. Cái 1.9.0 làm được là
**bỏ hẳn phần "không biết là có bản mới"**.

### 1. Banner tự hiện ở tab Chạy

Mở Side Panel là tiện ích tự đọc một file `version.json` nhỏ trên GitHub, so với
version trong `manifest.json`. Mới hơn thì hiện banner ngay đầu tab Chạy:

```
⬆ Có bản mới 1.9.1
Sửa lỗi lệch tên file khi tải 4K. — (phát hành 2026-09-20, bạn đang dùng 1.9.0).
Xem Cài đặt → ⬆ Cập nhật tiện ích để biết 3 bước thay thư mục.
                                                    [Tải về]  [Để sau]
```

- **Tải về** mở thẳng link tải trong tab mới.
- **Để sau** ẩn banner cho **đúng bản đó**; bản sau vẫn báo lại. Không có kiểu tắt một
  lần rồi im mãi.
- Mở trình duyệt cũng kiểm tra một lượt (`onStartup`) và hiện **thông báo hệ thống**,
  nhưng chỉ **một lần cho mỗi bản** — không spam.

### 2. Cài đặt → ⬆ Cập nhật tiện ích

- **🔄 Kiểm tra bản mới** — bấm là kiểm tra ngay, bỏ qua throttle.
- **↗ Mở trang tải** — mở link tải của bản mới nhất.
- **Tự kiểm tra khi mở Side Panel và khi mở trình duyệt** — tắt được. Tắt chỉ tắt phần
  *tự động*; nút bấm tay vẫn dùng được bình thường.
- **Địa chỉ file version.json** — đổi được, để khi repo đổi tên/đổi chủ thì không phải
  build lại tiện ích. Chỉ nhận `https` trên `raw.githubusercontent.com` hoặc
  `gist.githubusercontent.com` (đúng những host đã khai trong `host_permissions`;
  host khác thì Chrome chặn fetch, nên tiện ích **chặn sớm và báo rõ** thay vì để lỗi
  mơ hồ). Để trống = dùng địa chỉ mặc định của dự án.
- Mục này ghi luôn **3 bước thay thư mục**, kèm một điều quan trọng: **cài đặt và dự án
  đã lưu KHÔNG bị mất** khi thay thư mục (chúng nằm trong `chrome.storage.local`), chỉ
  mất nếu gỡ hẳn tiện ích rồi cài lại trên profile Chrome khác.

### 3. Cách phát hành một bản mới (phía người giữ repo)

Sửa **một file duy nhất** ở gốc repo — `version.json`:

```json
{
  "version": "1.9.1",
  "downloadUrl": "https://github.com/RollReus6868/flow-automation/archive/refs/heads/main.zip",
  "notes": "Sửa lỗi lệch tên file khi tải 4K.",
  "publishedAt": "2026-09-20"
}
```

rồi `git add version.json && git commit && git push`. Trong vòng vài phút, mọi máy đang
chạy tiện ích sẽ thấy banner. Người nhận tải `main.zip`, giải nén, và Load unpacked thư
mục `ext/flow-automation-local-1.9.1` bên trong.

### Những chỗ đã lường trước (đều có test)

- **So sánh version bằng số, không bằng chuỗi.** `1.10.0` phải MỚI HƠN `1.9.0`, nhưng so
  chuỗi thì `"1.10.0" < "1.9.0"` — đây là lỗi kinh điển của loại tính năng này.
  Chấp nhận cả 2–4 phần và đuôi pre-release (`1.9.0-beta.2`).
- **Chống cache.** `raw.githubusercontent.com` trả `Cache-Control: max-age=300`; thiếu
  tham số chống cache thì vừa push xong vẫn đọc ra bản cũ.
- **Throttle 6 giờ.** Mở/đóng panel liên tục không gọi mạng liên tục; bấm nút tay thì
  bỏ qua throttle.
- **Trạng thái nằm trong `chrome.storage.local`, không nằm trong biến của worker** — MV3
  kill service worker khi rảnh, giữ trong biến là mất.
- **Mọi lỗi mạng đều có chữ tiếng Việt cụ thể** (mất mạng / hết 10 giây chờ / mã 404 khi
  repo đổi tên / `version.json` sai cú pháp), hiện ngay trong mục Cài đặt — không im lặng.
- **Panel không bao giờ treo vì background.** Mỗi lời gọi tới service worker có thời gian
  chờ 5 giây, và phần kiểm tra bản mới **không chặn** khởi tạo panel. Bản nháp đầu tiên
  chặn thật: worker không phản hồi là panel đứng im, không bấm được tab nào — bắt được
  đúng hiện tượng này khi chụp ảnh giao diện bằng Chromium thật, nên giờ có test riêng
  cho nó.
- **Chặn host đánh lừa** như `raw.githubusercontent.com.evil.tld`.

---

## 0c. Bản 1.8.0 — TỰ CHẨN ĐOÁN KHI FLOW ĐỔI GIAO DIỆN (vẫn còn)

Tiện ích này **không dùng API chính thức nào** của Google Flow (Google không cung cấp API
cho việc này). Nó hoạt động bằng cách “đọc” giao diện web: tìm ô nhập, nút bấm, thẻ video
qua class/thuộc tính HTML — riêng `content-v2.js` có ~175 chỗ tìm phần tử DOM. Nên **mỗi
lần Google đổi giao diện là một số bước có thể trượt**, và trước 1.8.0 biểu hiện chỉ là
“bot đứng im” hoặc timeout chung chung, rất khó biết vỡ ở đâu.

Bản 1.8.0 không ngăn được việc Google đổi UI (không ai ngăn được), nhưng làm ba việc để
bạn **tự vá trong vài phút** thay vì phải chờ:

### 1. Báo rõ vỡ ở bước nào

Sáu phần tử then chốt được đặt tên tiếng Việt và theo dõi riêng: *Ô nhập prompt*,
*Nút Tạo*, *Ô tìm kiếm*, *Thẻ video/ảnh*, *Nút ⋮ trên thẻ*, *Nút cài đặt mô hình*.
Dò trượt 3 lần liên tiếp thì Log báo thẳng:

```
🔧 KHÔNG TÌM THẤY "Ô nhập prompt" sau 3 lần thử — rất có thể Google Flow đã đổi giao diện
ở bước này. Mở Cài đặt → Chẩn đoán giao diện Flow: bấm "Chọn trên trang" để tự chỉ lại
phần tử này, hoặc "Tải báo cáo .txt" rồi gửi cho Claude để vá.
```

Tab Chạy cũng hiện banner đỏ kèm nút **Mở chẩn đoán**. Có chống báo động sai: lưới trống
là chuyện bình thường (dự án mới), nên chỉ tính là “vỡ” khi đúng ra phần tử đó phải tồn
tại; và mỗi phần tử chỉ báo một lần mỗi 5 phút để không spam Log.

### 2. Tự chọn lại selector — không cần chờ bản cập nhật

*Cài đặt → 🔧 Chẩn đoán giao diện Flow*:

- **🔍 Kiểm tra giao diện** — quét một lượt, hiện ✅/❌ từng phần tử, kèm số lượng tìm
  thấy và đang dùng cách dò mặc định hay selector của bạn. Bật **Kiểm tra sâu** thì nó
  mở thử menu ⋮ của một thẻ để đọc tên các mục (“Tải xuống”, “Đổi tên”, “Tăng lên 4K”…)
  rồi tự Escape — chữ trên menu cũng là thứ Google có thể đổi. Nếu bot đang thao tác thì
  bước này tự hoãn để không phá luồng đang chạy.
- **🎯 Chọn trên trang** — bật chế độ chọn: trang Flow hiện dải hướng dẫn ở trên, bạn
  hover thấy viền xanh, click đúng phần tử là xong (Esc để huỷ). Selector bạn chọn được
  **ưu tiên tuyệt đối** so với cách dò mặc định.
- **⌨ Nhập tay** — dán trực tiếp selector CSS nếu bạn tự xem F12. Sai cú pháp bị chặn
  ngay tại chỗ, không lưu vào để làm automation lỗi.
- Phần tử ❌ còn kèm danh sách **ứng viên trên trang** — bấm một dòng là dùng luôn
  selector đó, không cần tự viết.
- **Xoá** để quay về cách dò mặc định.

Selector sinh ra được viết lại cho **bền**: không bám id do framework sinh
(`mat-menu-panel-12`, `radix-:r7:`), không bám class hash build (`x1f6kntn9`), ưu tiên
`aria-label` / `role` / `data-testid` / tên custom element (`flow-tile-container`), và
luôn **kiểm chứng lại trên DOM thật** trước khi lưu. Selector cho nút ⋮ được sinh **tương
đối trong phạm vi thẻ** (bản cũ sinh đường dẫn từ `<body>` nên không khớp khi quét trong
từng thẻ).

### 3. Báo cáo .txt đủ để vá ngay

**⬇ Tải báo cáo .txt** xuất một file gồm: phiên bản tiện ích, URL trang, số thẻ trên
lưới, trạng thái từng phần tử, selector đang dùng, **danh sách phần tử ứng viên kèm
HTML rút gọn và selector đề xuất**, các mục đọc được trong menu ⋮, cài đặt liên quan và
100 dòng log cuối. Gửi đúng file đó là đủ để viết lại selector mới — bạn **không cần tự
mở F12** và không phải qua lại nhiều lượt.

---

## 0d. Bản 1.7.0 — CHẠY NHIỀU TAB (vẫn còn)
...
3. **Load unpacked** → trỏ tới thư mục `flow-automation-local-1.10.0`.
